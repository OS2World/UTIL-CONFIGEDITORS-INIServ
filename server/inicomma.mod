...
PROCEDURE SetApp (session: Session;  VAR (*IN*) value: ARRAY OF CHAR);
    ...
    BEGIN
        Strings.Assign (value, session^.Application);
        Reply (session, "+");
    END SetApp;

(********************************************************************************)

PROCEDURE ChangeDir (session: Session;  VAR (*IN*) newdir: ARRAY OF CHAR);
...
PROCEDURE ChooseFile (session: Session;  VAR (*IN*) filename: ARRAY OF CHAR);
    ...
    BEGIN
        Strings.Assign (filename, session^.filename);
        Reply (session, "+");
    END ChooseFile;
...
PROCEDURE SetKey (session: Session;  VAR (*IN*) value: ARRAY OF CHAR);
    ...
    BEGIN
        Strings.Assign (value, session^.Key);
        Reply (session, "+");
    END SetKey;
...
PROCEDURE ListDir (session: Session;  VAR (*IN*) dirname: ARRAY OF CHAR);
    ...
    VAR mask: FilenameString;  AtRoot: BOOLEAN;

    BEGIN
        MakeFilename (session^.CurrentDir, dirname, mask, AtRoot);
        Reply (session, "+");
        IF AtRoot THEN
...
PROCEDURE MakeDir (session: Session;  VAR (*IN*) newdir: ARRAY OF CHAR);
    ...
    BEGIN
        IF MakeDirectory (session^.CurrentDir, newdir) THEN
            Reply (session, "+");
        ELSE
            Reply (session, '-');
        END (*IF*);
...
PROCEDURE Relocate (session: Session;  VAR (*IN*) dest: ARRAY OF CHAR);
    ...
    BEGIN
        IF MoveFile (session^.CurrentDir, session^.Application, dest) THEN
            Reply (session, "+");
        ELSE
            Reply (session, '-');
        END (*IF*);
